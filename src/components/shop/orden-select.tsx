...
export function OrdenSelect({ valorActual }: { valorActual: string }) {
  ...
  return (
    <label className="flex items-center gap-2 text-[11px] uppercase tracking-[0.12em] text-piedra-oscura">
      Ordenar por
      <select
        value={valorActual}
        onChange={(evento) => {
        ...
        className="rounded-marca border border-piedra/45 bg-white px-2.5 py-1.5 text-xs normal-case tracking-normal text-carbon focus:border-acento-fuerte focus:outline-none"
      >
        {ORDENES.map((orden) => (
