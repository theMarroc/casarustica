  ...
  return (
    <form action={enviar} className="flex flex-col gap-5">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        >
          <Campo name="motivo" maxLength={120} placeholder="Aumento de la madera, junio" />
        </CampoConEtiqueta>
      </div>

      <div className="rounded-marca border border-piedra/30 bg-lino/50">
        <p className="border-b border-piedra/25 px-4 py-2.5 text-[11px] font-semibold uppercase tracking-[0.12em] text-nogal">
          Vista previa: {cambian.length}{" "}
          {cambian.length === 1 ? "precio cambia" : "precios cambian"}
        </p>

        {cambian.length === 0 ? (
          <p className="px-4 py-6 text-center text-sm text-piedra-oscura">
            Con esos valores no cambia ningún precio.
          </p>
        ) : (
          <ul className="max-h-64 divide-y divide-piedra/15 overflow-y-auto">
            {cambian.map((producto) => (
              ...
              >
                ...
                <span className="flex shrink-0 items-center gap-2 tabular-nums">
                  <span className="text-piedra-oscura line-through">
                    {formatARS(producto.price)}
                  </span>
                  <ArrowRight className="h-3.5 w-3.5 text-piedra-oscura" />
      ...
      <p className="text-xs leading-relaxed text-piedra-oscura">
