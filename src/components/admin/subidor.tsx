  ...
  return (
    <div className="flex flex-col">
      <span className="mb-1.5 flex flex-col gap-0.5">
        <span className="text-[11px] font-semibold uppercase tracking-[0.1em] text-nogal">
          {etiqueta}
        </span>
        {ayuda ? <span className="text-xs text-piedra-oscura">{ayuda}</span> : null}
      </span>

      <input type="hidden" name={nombre} value={url} />

      {url ? (
        <div className="relative h-40 w-full overflow-hidden rounded-marca border border-piedra/30 bg-arena/20 sm:w-64">
          <Image src={url} alt="" fill sizes="256px" className="object-cover" />
          <button
            type="button"
            onClick={() => setUrl("")}
            className="absolute right-2 top-2 rounded-full bg-white/90 p-1.5 text-acento-fuerte shadow-sm transition-colors hover:bg-white"
        ...
        <label
          className={cn(
            "flex h-40 w-full cursor-pointer flex-col items-center justify-center gap-2 rounded-marca border border-dashed border-piedra/60 bg-lino/50 text-center transition-colors hover:border-acento-fuerte hover:bg-acento-fuerte/5 sm:w-64",
            subiendo && "pointer-events-none opacity-60",
          )}
        >
          {subiendo ? (
            <Loader2 className="h-6 w-6 animate-spin text-piedra-oscura" />
          ) : (
            <ImagePlus className="h-6 w-6 text-piedra-oscura" strokeWidth={1.3} />
          )}
          <span className="text-xs font-semibold text-nogal">
      ...
      {error ? (
        <p className="mt-2 text-xs text-alerta-oscura">{error}</p>
      ) : null}
    </div>
  );
}
...
export function GestorImagenes({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {imagenes.length > 0 ? (
        <ul className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {imagenes.map((imagen, indice) => (
            <li
              key={imagen.url}
              className="overflow-hidden rounded-marca border border-piedra/30 bg-white"
            >
              ...
              <div className="flex items-center justify-between gap-1 p-1.5">
                <div className="flex gap-0.5">
                  <button
                    ...
                    disabled={indice === 0}
                    className="rounded-marca p-1.5 text-piedra-oscura transition-colors hover:bg-arena/30 hover:text-nogal disabled:opacity-30"
      ...
      <label
        className={cn(
          "flex cursor-pointer flex-col items-center justify-center gap-2 rounded-marca border border-dashed border-piedra/60 bg-lino/50 px-4 py-8 text-center transition-colors hover:border-acento-fuerte hover:bg-acento-fuerte/5",
          subiendo && "pointer-events-none opacity-60",
        )}
      >
        {subiendo ? (
      ...
      {error ? <p className="text-xs text-alerta-oscura">{error}</p> : null}
    </div>
  );
}
