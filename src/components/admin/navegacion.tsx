...
export function NavegacionAdmin({ nombre }: { nombre: string }) {
    ...
    <>
      <div className="flex flex-col gap-6">
        {SECCIONES.map((seccion) => (
          <div key={seccion.titulo}>
            <p className="mb-2 px-3 text-[10px] font-bold uppercase tracking-[0.14em] text-piedra-oscura">
              {seccion.titulo}
            </p>
      ...
      <div className="mt-8 border-t border-piedra/30 pt-4">
        ...
        <Link
          href="/"
          target="_blank"
          className="flex items-center gap-2.5 rounded-marca px-3 py-2 text-sm text-carbon/75 transition-colors hover:bg-arena/30 hover:text-carbon"
        >
        ...
        <form action={salir}>
          <button
            type="submit"
            className="flex w-full items-center gap-2.5 rounded-marca px-3 py-2 text-sm text-carbon/75 transition-colors hover:bg-acento-fuerte/10 hover:text-acento-fuerte"
          >
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between border-b border-piedra/25 bg-lino px-4 py-3 lg:hidden">
        <Link href="/admin">
          <Logo />
        </Link>
        <button
          onClick={() => setAbierto((v) => !v)}
          className="rounded-marca p-2 text-carbon/70 hover:bg-arena/25"
      ...
      {abierto ? (
        <nav className="border-b border-piedra/25 bg-lino px-3 py-4 lg:hidden">
          {contenido}
        </nav>
      ) : null}

      {/* Barra lateral en escritorio */}
      <nav className="sticky top-0 hidden h-dvh flex-col overflow-y-auto border-r border-piedra/25 bg-lino px-3 py-6 lg:flex">
        <Link href="/admin" className="mb-8 px-3">
          <Logo />
          <span className="mt-1 block text-[10px] uppercase tracking-[0.16em] text-piedra-oscura">
            Panel
          </span>
        </Link>
        {contenido}
