...
export function SubirComprobante({
  ...
  if (fase === "listo") {
    return (
      <div className="flex items-start gap-3 rounded-marca border border-salvia/40 bg-salvia/10 p-5">
        <CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-salvia" strokeWidth={1.5} />
        <div>
  ...
  return (
    <form
      onSubmit={enviar}
      className="rounded-marca border border-piedra/30 bg-white p-6 shadow-suave"
    >
      ...
      <label
        className={`mt-5 flex cursor-pointer flex-col items-center gap-2 rounded-marca border border-dashed border-piedra/60 bg-lino/50 px-4 py-8 text-center transition-colors hover:border-acento-fuerte hover:bg-acento-fuerte/5 ${
          ocupado ? "pointer-events-none opacity-60" : ""
        }`}
      >
        <Upload className="h-7 w-7 text-piedra-oscura" strokeWidth={1.3} />
      ...
      {error ? (
        <p
          aria-live="polite"
          className="mt-3 flex items-start gap-2 rounded-marca border border-alerta/40 bg-alerta/10 px-3 py-2 text-xs leading-relaxed text-alerta-oscura"
        >
