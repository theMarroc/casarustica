...
export function Carrusel({
  ...
  return (
    <section className="overflow-hidden bg-lino py-16 lg:py-20">
      ...
      <div className="group mt-10 overflow-hidden">
        <div className="marquesina flex gap-4 group-hover:[animation-play-state:paused]">
          {tira.map((foto, indice) => (
            <figure
              key={`${foto.id}-${indice}`}
              className="relative h-56 w-44 shrink-0 overflow-hidden rounded-marca bg-arena/25 sm:h-72 sm:w-56"
            >
              <Image
                src={foto.url}
                alt={foto.caption ?? ""}
