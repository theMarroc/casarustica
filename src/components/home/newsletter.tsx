...
export function Newsletter({ ajustes }: { ajustes: Settings }) {
  ...
  return (
    <section className="relative overflow-hidden bg-acento py-12">
          ...
          <form action={accion} className="flex flex-col gap-2">
            <div className="flex">
              ...
              <input
                ...
                className="min-w-0 flex-1 rounded-l-marca border-0 bg-white px-3.5 py-3 text-sm text-carbon placeholder:text-piedra-oscura focus:outline-none focus:ring-2 focus:ring-inset focus:ring-carbon/30"
              />
              <button
                type="submit"
                disabled={enviando}
                className="rounded-r-marca bg-carbon px-5 text-[11px] font-semibold uppercase tracking-[0.12em] text-hueso transition-colors hover:bg-carbon/85 disabled:opacity-60"
              >
            ...
            {estado ? (
              <p
                ...
                className={`text-xs font-semibold ${
                  estado.ok ? "text-salvia" : "text-alerta-oscura"
                }`}
              >
                {estado.mensaje}
              </p>
          ...
          <div className="flex gap-2 lg:justify-end">
            {instagram ? (
              <a
                ...
                className="rounded-marca bg-white/70 p-2.5 text-nogal transition-colors hover:bg-white"
              >
                <IconoInstagram />
              </a>
            ) : null}
