      ...
      {direcciones.length > 0 ? (
        <ul className="flex flex-col gap-3">
          {direcciones.map((direccion) => (
            <li
              key={direccion.id}
              className="flex flex-wrap items-start justify-between gap-4 rounded-marca border border-piedra/30 bg-white p-4 shadow-suave"
            >
              <div className="flex gap-3">
                <Home className="mt-0.5 h-5 w-5 shrink-0 text-piedra-oscura" strokeWidth={1.4} />
                <div>
                  <p className="flex items-center gap-2 font-semibold text-nogal">
                    {direccion.label}
                    {direccion.is_default ? (
                      <Insignia className="bg-acento/40 text-nogal">Principal</Insignia>
                    ) : null}
                  </p>
                  ...
                  {direccion.notes ? (
                    <p className="mt-1 text-xs text-piedra-oscura">{direccion.notes}</p>
                  ) : null}
                </div>
              </div>
              ...
              <div className="flex items-center gap-1">
                {!direccion.is_default ? (
                  <form action={marcarPredeterminada}>
                    ...
                    <button
                      ...
                      className="rounded-marca p-2 text-piedra-oscura transition-colors hover:bg-acento/20 hover:text-nogal"
                    >
                ...
                <button
                  ...
                  title="Editar"
                  className="rounded-marca p-2 text-piedra-oscura transition-colors hover:bg-arena/25 hover:text-nogal"
                >
      ...
      {editando ? (
        ...
        >
          ...
          <div className="mb-4 flex items-center justify-between">
            <h3 className="font-display text-lg text-nogal">
            ...
            {direcciones.length > 0 ? (
              <button
                ...
                className="rounded-marca p-1.5 text-piedra-oscura transition-colors hover:text-nogal"
                aria-label="Cancelar"
              >
                <X className="h-4 w-4" />
              </button>
          ...
          {estado && !estado.ok ? (
            <p className="mt-3 rounded-marca border border-alerta/40 bg-alerta/10 px-3 py-2 text-xs text-alerta-oscura">
              {estado.mensaje}
            </p>
          ) : null}
