  ...
  return (
    <>
      ...
      <aside
        ...
        className={`fixed inset-y-0 right-0 z-50 flex w-full max-w-sm flex-col bg-hueso shadow-tarjeta transition-transform duration-300 ${
          panelAbierto ? "translate-x-0" : "translate-x-full"
        }`}
      >
        <header className="flex items-center justify-between border-b border-piedra/30 px-5 py-4">
          <h2 className="font-display text-xl text-nogal">Tu pedido</h2>
          <button
            onClick={cerrarPanel}
            className="rounded-marca p-1.5 text-piedra-oscura transition-colors hover:bg-arena/25 hover:text-nogal"
        ...
        {items.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-4 px-6 text-center">
            <ShoppingBag className="h-10 w-10 text-piedra-oscura/60" strokeWidth={1.2} />
            <p className="text-sm text-piedra-oscura">Todavía no agregaste nada.</p>
            <Link
              href="/tienda"
              onClick={cerrarPanel}
          ...
          <>
            <ul className="flex-1 divide-y divide-piedra/20 overflow-y-auto px-5">
              {items.map((item) => (
                <li key={item.clave} className="flex gap-3 py-4">
                  <div className="relative h-20 w-16 shrink-0 overflow-hidden rounded-marca bg-arena/25">
                    {item.imagen ? (
                      <Image
                        src={item.imagen}
                        alt={item.nombre}
                  ...
                  <div className="flex min-w-0 flex-1 flex-col">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <p className="truncate font-display text-base leading-snug text-nogal">
                          {item.nombre}
                        </p>
                        {item.unidad ? (
                    ...
                    <div className="mt-auto flex items-center justify-between pt-2">
                      <div className="flex items-center rounded-marca border border-piedra/40">
                        <button
                          onClick={() => cambiarCantidad(item.clave, item.cantidad - 1)}
                          className="px-2 py-1.5 text-nogal transition-colors hover:bg-arena/25"
                      ...
                      <div className="text-right">
                        ...
                        <p className="font-semibold text-nogal">
            ...
            <footer className="border-t border-piedra/30 bg-white px-5 py-4">
              {ahorro > 0 ? (
                <p className="mb-2 flex items-center justify-between text-sm text-salvia">
              ...
              <p className="mt-1 text-xs text-piedra-oscura">
