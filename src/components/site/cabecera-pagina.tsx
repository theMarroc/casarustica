...
export function CabeceraPagina({
  ...
  return (
    <section className="border-b border-piedra/25 bg-lino py-12 lg:py-16">
      <div className="contenedor flex flex-col items-center text-center">
        {migas && migas.length > 0 ? (
          <nav
            aria-label="Ubicación"
            className="mb-5 flex flex-wrap items-center justify-center gap-1 text-[11px] uppercase tracking-[0.12em] text-piedra-oscura"
          >
            ...
            {migas.map((miga) => (
              <span key={miga.href} className="flex items-center gap-1">
                ...
                <Link href={miga.href} className="transition-colors hover:text-nogal">
                  {miga.texto}
                </Link>
              </span>
            ))}
        ...
        <h1 className="titulo-seccion text-nogal">
          {titulo}
          {tituloCursiva ? (
            <>
              {" "}
