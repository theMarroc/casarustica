...
export async function generateMetadata({
  ...
  if (!combo) return { title: "Set no encontrado" };
    ...
    <>
      <div className="contenedor pt-8">
        ...
        >
          ...
          <Link href="/sets" className="transition-colors hover:text-nogal">
            Sets y kits
          </Link>
        </nav>
      </div>
      ...
      <article className="contenedor grid gap-10 py-8 lg:grid-cols-2 lg:gap-16 lg:py-12">
        <div className="relative aspect-square overflow-hidden rounded-marca border border-piedra/25 bg-arena/20">
          {combo.image_url ? (
            <Image
              src={combo.image_url}
              alt={combo.name}
        ...
        <div className="flex flex-col">
          <Insignia className="mb-3 self-start bg-acento text-carbon">Set</Insignia>
          ...
          <div className="mt-5 flex items-baseline gap-3">
            ...
            {ahorro > 0 ? (
              <p className="text-lg text-piedra-oscura line-through">{formatARS(precioSuelto)}</p>
            ) : null}
          </div>
          ...
          {incluidos.length > 0 ? (
            <div className="mt-7 rounded-marca border border-piedra/30 bg-white p-5">
              <h2 className="font-sans text-[11px] font-semibold uppercase tracking-[0.12em] text-nogal">
                Qué incluye
              </h2>
              <ul className="mt-3 flex flex-col gap-2 text-sm text-carbon/75">
                {incluidos.map(({ item, producto }) => (
                  <li key={item.id} className="flex items-start gap-2.5">
                    <Check
                      className="mt-0.5 h-4 w-4 shrink-0 text-salvia"
                      strokeWidth={2}
                    />
                    <span>
                      ...
                      {producto ? (
                        <Link
                          ...
                          className="underline decoration-piedra/50 underline-offset-2 transition-colors hover:text-acento-fuerte"
                        >
                          {producto.name}
                        </Link>
                      ) : (
