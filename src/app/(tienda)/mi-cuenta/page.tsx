    ...
    <>
      ...
      <div className="contenedor grid gap-10 py-10 lg:grid-cols-[1fr_22rem] lg:gap-14 lg:py-14">
        <div className="flex flex-col gap-10">
          <section>
            <h2 className="font-display text-2xl text-nogal">Mis pedidos</h2>
            <span className="linea-decorativa mt-3" />

            {!pedidos || pedidos.length === 0 ? (
              <div className="mt-6 flex flex-col items-center gap-4 rounded-marca border border-dashed border-piedra/50 bg-lino/60 px-6 py-12 text-center">
                <Package className="h-10 w-10 text-piedra-oscura/60" strokeWidth={1.1} />
                <p className="text-sm text-piedra-oscura">Todavía no hiciste ningún pedido.</p>
              ...
              <ul className="mt-6 flex flex-col gap-3">
                ...
                >[]).map((pedido) => {
                  ...
                  return (
                    <li
                      key={pedido.id}
                      className="flex flex-wrap items-center justify-between gap-4 rounded-marca border border-piedra/30 bg-white p-4 shadow-suave"
                    >
                      <div>
                        <p className="font-mono text-sm font-semibold tracking-wider text-nogal">
                          {pedido.code}
                        </p>
          ...
          <section>
            <h2 className="font-display text-2xl text-nogal">Mis direcciones</h2>
            <span className="linea-decorativa mt-3" />
        ...
        <aside className="h-fit rounded-marca border border-piedra/30 bg-white p-6 shadow-suave">
          <h2 className="font-display text-lg text-nogal">Tus datos</h2>
          <span className="linea-decorativa mt-3" />

          <dl className="mt-5 flex flex-col gap-3 text-sm">
            <div>
              <dt className="text-[10px] font-semibold uppercase tracking-[0.12em] text-piedra-oscura">
                Nombre
              </dt>
              <dd className="text-carbon/80">{perfil?.full_name || "Sin cargar"}</dd>
            </div>
            <div>
              <dt className="text-[10px] font-semibold uppercase tracking-[0.12em] text-piedra-oscura">
                Correo
            ...
            <div>
              ...
              <dd className="text-carbon/80">{perfil?.phone || "Sin cargar"}</dd>
            </div>
          </dl>
