...
function PantallaSinConfigurar() {
  const pasos = [
    ...
    "En Authentication › Users creá el usuario de Silvina con email y contraseña.",
    ...
    <div className="min-h-dvh bg-hueso">
      <div className="mx-auto max-w-2xl px-5 py-16">
        ...
        <h1 className="titulo-seccion mt-8 text-nogal">
          Falta conectar la <span className="cursiva-marca">base de datos</span>
        </h1>
        ...
        <ol className="mt-8 flex flex-col gap-4">
          {pasos.map((paso, indice) => (
            <li
              key={paso}
              className="flex gap-4 rounded-marca border border-piedra/30 bg-white p-4 shadow-suave"
            >
              <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-acento text-xs font-bold text-carbon">
                {indice + 1}
              </span>
        ...
        <p className="mt-8 text-sm text-piedra-oscura">
          Los pasos completos, con el detalle de qué copiar, están en el archivo{" "}
          <code className="rounded bg-lino px-1.5 py-0.5 font-mono text-xs">
            README.md
          </code>{" "}
          del proyecto.
        </p>
        ...
        <Link
          href="/"
          className="mt-8 inline-block text-[11px] font-semibold uppercase tracking-[0.12em] text-acento-fuerte hover:underline"
        >
          Volver al sitio
        </Link>
      </div>
