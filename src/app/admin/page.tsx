  ...
  return (
    <>
      ...
      {faltaConfigurar.length > 0 ? (
        <div className="mb-8 rounded-marca border border-acento/50 bg-acento/15 p-5">
          <h2 className="flex items-center gap-2 text-sm font-semibold text-nogal">
          ...
          <ul className="mt-3 flex flex-col gap-2">
            {faltaConfigurar.map((item) => (
              <li key={item.texto}>
                <Link
                  href={item.href}
                  className="flex items-center gap-1.5 text-sm text-carbon/80 transition-colors hover:text-acento-fuerte"
                >
                  {item.texto}
      ...
      <div className="mb-8 grid grid-cols-2 gap-4 lg:grid-cols-4">
        {tarjetas.map(({ titulo, valor, href, Icono, destacar }) => (
          <Link
            key={titulo}
            href={href}
            className={`rounded-marca border bg-white p-4 shadow-suave transition-colors hover:border-acento-fuerte/60 ${
              destacar ? "border-acento-fuerte/50" : "border-piedra/30"
            }`}
          >
            <Icono
              className={`h-5 w-5 ${destacar ? "text-acento-fuerte" : "text-piedra-oscura"}`}
              strokeWidth={1.5}
            />
            <p className="mt-3 font-display text-3xl text-nogal">{valor}</p>
      ...
      <PanelAdmin titulo="Últimos pedidos">
              ...
              return (
                <li key={pedido.id}>
                  <Link
                    ...
                    className="flex flex-wrap items-center justify-between gap-3 py-3 transition-colors hover:bg-lino/60"
                  >
                    <div>
        ...
        <Link
          href="/admin/pedidos"
          className="mt-4 inline-flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.12em] text-acento-fuerte hover:underline"
        >
