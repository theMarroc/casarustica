...
const esquema = z.object({
  ...
  name: z.string().trim().min(2, "Ponele un nombre al set."),
...
export async function guardarCombo(
  ...
  if (items.length === 0) {
    return { ok: false, mensaje: "Agregá al menos un producto al set." };
  }
  ...
  const fila = {
    ...
    ...(id ? {} : { slug: slugify(campos.name) || `set-${Date.now()}` }),
  };
  ...
  if (error || !guardado) {
    ...
    return {
      ok: false,
      mensaje: duplicado
        ? "Ya existe un set con ese nombre."
  ...
  return { ok: true, mensaje: "Set guardado." };
}
